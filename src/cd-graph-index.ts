...
type ContentTypeDescription = {
	...
	rootArguments: IntrospectionArgument[];
	resultTypeName: string;
	itemTypeName: string;
	itemFields: IntrospectionField[];
};

type GeneratedExecution = {
	mode: 'content-list' | 'content-type-list' | 'content-type-detail';
	request: string;
	query: string;
...
function ensureGraphConfig(): void {
	if (!GRAPH_ENDPOINT) {
...
async function executeGraphQuery<T = JsonObject>(query: string, variables?: JsonObject): Promise<T> {
	ensureGraphConfig();
	...
	try {
		parsedBody = bodyText ? (JSON.parse(bodyText) as JsonObject) : {};
	} catch (error) {
		throw new Error(`Graph response was not valid JSON: ${(error as Error).message}`);
	}
...
async function getCachedValue<T>(key: string, factory: () => Promise<T>): Promise<T> {
...
function typeToString(type: TypeRef | null | undefined): string {
	if (!type) {
		return 'Unknown';
	}
...
async function getRootQueryFields(): Promise<IntrospectionField[]> {
	return getCachedValue('root-fields', async () => {
		const result = await executeGraphQuery<{
			data?: { __schema?: { queryType?: { fields?: IntrospectionField[] } } };
			errors?: unknown;
		}>(ROOT_FIELDS_QUERY);
...
async function getTypeDetails(typeName: string): Promise<IntrospectionType> {
	return getCachedValue(`type:${typeName}`, async () => {
		const result = await executeGraphQuery<{
			data?: { __type?: IntrospectionType | null };
			errors?: unknown;
		}>(TYPE_DETAILS_QUERY, { name: typeName });
...
async function listContentTypes(search?: string): Promise<IntrospectionField[]> {
...
async function describeContentType(typeName: string): Promise<ContentTypeDescription> {
	const rootFields = await getRootQueryFields();
...
async function saveGeneratedQuery(query: string): Promise<string> {
	await ensureDirectories();
	await writeFile(TEMP_QUERY_FILE, `${query.trim()}\n`, 'utf8');
	return TEMP_QUERY_FILE;
}

function resolveDataSavePath(overridePath?: string): string {
	const selectedPath =
...
async function saveResponse(payload: JsonObject, overridePath?: string): Promise<string> {
...
function isScalarOrEnumField(field: IntrospectionField): boolean {
...
function scoreTypeName(typeName: string, request: string): number {
...
async function inferContentTypeName(request: string): Promise<string | undefined> {
...
function pickFields(request: string, fields: IntrospectionField[]): string[] {
...
function parseLimit(request: string, explicitLimit?: number): number {
...
function extractLocale(request: string): string | undefined {
	const localeMatch = request.match(/\b([a-z]{2}(?:-[A-Z]{2})?)\b/);
	return localeMatch?.[1];
}

function buildArgumentString(args: string[]): string {
	return args.length > 0 ? `(${args.join(', ')})` : '';
}

function escapeGraphString(input: string): string {
	return input.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
}

function buildListContentTypesQuery(): GeneratedExecution {
	return {
...
function buildContentTypeDetailsQuery(typeName: string, request: string): GeneratedExecution {
	return {
...
async function buildContentItemsQuery(request: string, typeName: string, explicitLimit?: number): Promise<GeneratedExecution> {
...
async function buildExecutionFromNaturalLanguage(request: string, explicitLimit?: number): Promise<GeneratedExecution> {
...
async function readReferenceLinks(): Promise<string> {
...
async function runAndPersistQuery(params: {
	...
	query: string;
	variables?: JsonObject;
	saveQueryFile?: boolean;
	outputPath?: string;
	metadata?: JsonObject;
}): Promise<{ responsePath: string; queryPath?: string; response: JsonObject }> {
	const queryPath = params.saveQueryFile === false ? undefined : await saveGeneratedQuery(params.query);
	const response = await executeGraphQuery<JsonObject>(params.query, params.variables);
...
function formatPathForUser(filePath: string): string {
	return path.relative(PROJECT_ROOT, filePath).replace(/\\/g, '/');
}

async function handleListContentTypes(args: JsonObject): Promise<string> {
...
async function handleDescribeContentType(args: JsonObject): Promise<string> {
...
async function handleExecuteQuery(args: JsonObject): Promise<string> {
	...
	if (typeof args.variablesJson === 'string' && args.variablesJson.trim()) {
		variables = JSON.parse(args.variablesJson) as JsonObject;
	} else if (args.variables && typeof args.variables === 'object' && !Array.isArray(args.variables)) {
		variables = args.variables as JsonObject;
	}
...
async function handleNaturalLanguageQuery(args: JsonObject): Promise<string> {
...
async function handleReferenceLinks(): Promise<string> {
	...
	return [
		{
			name: 'graph_nl_query',
			description:
				'Parse a natural-language request into an Optimizely Graph query or introspection query, execute it, optionally overwrite query/temp.graphql with the generated query, and save the response to DATA_SAVE_PATH (or out/data.json by default).',
			inputSchema: {
				type: 'object',
				properties: {
					request: {
		...
		},
		{
			...
			description:
				'Read available Optimizely SaaS CMS content types from the Optimizely Graph schema using Graph introspection only.',
			inputSchema: {
				type: 'object',
				properties: {
					search: {
		...
		},
		{
			...
			description:
				'Describe a specific Optimizely Graph content type, including root arguments and available item fields.',
			inputSchema: {
				type: 'object',
				properties: {
					typeName: {
		...
		},
		{
			...
			description:
				'Execute a raw Optimizely Graph query directly, optionally overwrite query/temp.graphql with the query, and always persist the response to DATA_SAVE_PATH (or out/data.json by default).',
			inputSchema: {
				type: 'object',
				properties: {
					query: {
		...
		},
		{
			...
			description:
				'Read the local Graph reference links from rag/document.txt when documentation context is needed.',
			inputSchema: {
				type: 'object',
				properties: {},
				additionalProperties: false,
...
async function runNaturalLanguageFromCli(request: string): Promise<void> {
