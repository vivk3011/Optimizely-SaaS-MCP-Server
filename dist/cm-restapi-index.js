...
// ============================================================================
async function makeRestRequest(method, endpoint, body, options) {
...
async function saveFilteredResponse(data) {
    const outputPath = resolveDataSavePath();
    await mkdir(path.dirname(outputPath), { recursive: true });
    await writeFile(outputPath, JSON.stringify(data, null, 2));
}
function resolveDataSavePath() {
...
function normalizePropertyDefinition(rawDefinition) {
    ...
    const mediaOnly = definition.mediaOnly === true ||
        definition.onlyMedia === true ||
        definition.allowOnlyMedia === true;
    if (definition.type === 'contentReference' &&
        mediaOnly &&
...
function buildPropertiesFromFields(fields) {
    ...
    for (const field of fields) {
        ...
        if (typeof field.localized === 'boolean')
            definition.isLocalized = field.localized;
        if (field.group)
            definition.group = field.group;
        if (typeof field.sortOrder === 'number')
            definition.sortOrder = field.sortOrder;
        if (field.allowedTypes?.length)
            definition.allowedTypes = field.allowedTypes;
        if (field.restrictedTypes?.length)
            definition.restrictedTypes = field.restrictedTypes;
        const mediaOnly = field.mediaOnly || field.onlyMedia || field.allowOnlyMedia;
        if (normalizedType === 'contentReference' &&
            mediaOnly &&
            (!Array.isArray(definition.allowedTypes) || definition.allowedTypes.length === 0)) {
            definition.allowedTypes = ['_media'];
        }
...
async function get_content_type(input) {
    const result = await makeRestRequest('GET', `/contenttypes/${input.key}`);
    await saveFilteredResponse(result);
    return result;
}
async function list_content_types(input = {}) {
    let endpoint = '/contenttypes';
    const params = new URLSearchParams();
    if (input.limit)
        params.append('pageSize', input.limit.toString());
    if (params.toString()) {
    ...
    if (input.search && typeof result === 'object' && result !== null && 'items' in result) {
        ...
        const filtered = items.filter(ct => (ct.key?.toLowerCase().includes(input.search.toLowerCase()) ||
...
async function update_content_type(input) {
    ...
    const result = await makeRestRequest('PATCH', endpoint, payload, { headers: { 'Content-Type': 'application/merge-patch+json' } });
    await saveFilteredResponse(result);
    return result;
}
...
async function delete_content_type(input) {
...
async function get_content(input) {
...
async function update_content_version(input) {
    ...
    const result = await makeRestRequest('PATCH', `/content/${input.contentKey}/versions/${input.versionId}`, payload, { headers: { 'Content-Type': 'application/merge-patch+json' } });
    await saveFilteredResponse(result);
    return result;
}
async function publish_content(input) {
    const payload = {};
    if (input.delayUntil)
        payload.delayUntil = input.delayUntil;
    if (input.force)
        payload.force = input.force;
    const result = await makeRestRequest('POST', `/content/${input.contentKey}/versions/${input.versionId}:publish`, payload);
    await saveFilteredResponse(result);
    return result;
}
async function ready_content(input) {
    ...
    const result = await makeRestRequest('POST', `/content/${input.contentKey}/versions/${input.versionId}:ready`, payload);
    await saveFilteredResponse(result);
    return result;
}
async function draft_content(input) {
    const result = await makeRestRequest('POST', `/content/${input.contentKey}/versions/${input.versionId}:draft`);
    await saveFilteredResponse(result);
    return result;
}
...
async function create_media(input) {
    ...
    formData.append('content', new Blob([JSON.stringify(contentPart)], { type: 'application/json' }), 'content.json');
    // Append file
    formData.append('file', new Blob([fileData], { type: input.fileType || 'application/octet-stream' }), fileName);
...
async function get_blueprint(input) {
    const result = await makeRestRequest('GET', `/blueprints/${input.key}`);
    await saveFilteredResponse(result);
    return result;
}
async function list_blueprints(input = {}) {
...
async function update_blueprint(input) {
    ...
    if (input.displayName)
        payload.displayName = input.displayName;
    if (input.properties)
        payload.content = { properties: input.properties };
    const result = await makeRestRequest('PATCH', `/blueprints/${input.key}`, payload, { headers: { 'Content-Type': 'application/merge-patch+json' } });
    await saveFilteredResponse(result);
    return result;
}
async function delete_blueprint(input) {
...
async function get_content_source(input) {
    const result = await makeRestRequest('GET', `/contentsources/${input.key}`);
    await saveFilteredResponse(result);
    return result;
}
async function list_content_sources(input = {}) {
...
async function update_content_source(input) {
    ...
    if (input.displayName)
        payload.displayName = input.displayName;
    if (input.propertyMappings)
        payload.propertyMappings = input.propertyMappings;
    const result = await makeRestRequest('PATCH', `/contentsources/${input.key}`, payload, { headers: { 'Content-Type': 'application/merge-patch+json' } });
    await saveFilteredResponse(result);
    return result;
}
async function delete_content_source(input) {
...
async function get_content_type_binding(input) {
    const result = await makeRestRequest('GET', `/contenttypebindings/${input.key}`);
    await saveFilteredResponse(result);
    return result;
}
async function list_content_type_bindings(input = {}) {
...
async function update_content_type_binding(input) {
    ...
    if (input.propertyMappings)
        payload.propertyMappings = input.propertyMappings;
    const result = await makeRestRequest('PATCH', `/contenttypebindings/${input.key}`, payload, { headers: { 'Content-Type': 'application/merge-patch+json' } });
    await saveFilteredResponse(result);
    return result;
}
async function delete_content_type_binding(input) {
    ...
    try {
        ...
        switch (name) {
            case 'cma_create_content_type':
                result = await create_content_type(args);
                break;
            case 'cma_get_content_type':
                result = await get_content_type(args);
                break;
            case 'cma_list_content_types':
                result = await list_content_types(args);
                break;
            case 'cma_update_content_type':
                result = await update_content_type(args);
                break;
            case 'cma_upsert_content_type':
                result = await upsert_content_type(args);
                break;
            case 'cma_delete_content_type':
                result = await delete_content_type(args);
                break;
            case 'cma_create_content':
                result = await create_content(args);
                break;
            case 'cma_get_content':
                result = await get_content(args);
                break;
            case 'cma_list_content_items':
                result = await list_content_items(args);
                break;
            case 'cma_get_content_versions':
                result = await get_content_versions(args);
                break;
            case 'cma_get_content_version':
                result = await get_content_version(args);
                break;
            case 'cma_create_content_version':
                result = await create_content_version(args);
                break;
            case 'cma_update_content_version':
                result = await update_content_version(args);
                break;
            case 'cma_publish_content':
                result = await publish_content(args);
                break;
            case 'cma_ready_content':
                result = await ready_content(args);
                break;
            case 'cma_draft_content':
                result = await draft_content(args);
                break;
            case 'cma_delete_content':
                result = await delete_content(args);
                break;
            case 'cma_create_media':
                result = await create_media(args);
                break;
            case 'cma_get_media':
                result = await get_media(args);
                break;
            case 'cma_create_blueprint':
                result = await create_blueprint(args);
                break;
            case 'cma_get_blueprint':
                result = await get_blueprint(args);
                break;
            case 'cma_list_blueprints':
                result = await list_blueprints(args);
                break;
            case 'cma_update_blueprint':
                result = await update_blueprint(args);
                break;
            case 'cma_delete_blueprint':
                result = await delete_blueprint(args);
                break;
            case 'cma_create_content_source':
                result = await create_content_source(args);
                break;
            case 'cma_get_content_source':
                result = await get_content_source(args);
                break;
            case 'cma_list_content_sources':
                result = await list_content_sources(args);
                break;
            case 'cma_update_content_source':
                result = await update_content_source(args);
                break;
            case 'cma_delete_content_source':
                result = await delete_content_source(args);
                break;
            case 'cma_create_content_type_binding':
                result = await create_content_type_binding(args);
                break;
            case 'cma_get_content_type_binding':
                result = await get_content_type_binding(args);
                break;
            case 'cma_list_content_type_bindings':
                result = await list_content_type_bindings(args);
                break;
            case 'cma_update_content_type_binding':
                result = await update_content_type_binding(args);
                break;
            case 'cma_delete_content_type_binding':
                result = await delete_content_type_binding(args);
                break;
